...
export type AuthAppleLoginRequest = {
  accessToken: string;
};

export type AuthAppleLoginResponse = Tokens & {
  user: User;
};

export function useAuthAppleLoginService() {
  const fetchBase = useFetchBase();

  return useCallback(
    (data: AuthAppleLoginRequest, requestConfig?: RequestConfigType) => {
      return fetchBase(`${API_URL}/v1/auth/apple/login`, {
        method: 'POST',
        body: JSON.stringify(data),
        ...requestConfig,
      }).then(wrapperFetchJsonResponse<AuthAppleLoginResponse>);
    },
    [fetchBase],
  );
}
