...
export class AuthService {
  ...
  constructor(
    ...
    //   window: [
    //     this.configService.getOrThrow<number>('security.totpWindowPast', {
    //       infer: true,
    //     }),
    //     this.configService.getOrThrow<number>('security.totpWindowFuture', {
    //       infer: true,
    //     }),
    //   ],
    //   keyEncoder,
  ...
  async logout(data: Pick<JwtRefreshPayloadType, 'sessionId'>) {
    return this.sessionsService.softDelete({
      id: data.sessionId,
    });
  }
  ...
  getTotpQrCode(userId: number): Promise<string> {
    ...
    const otpauth = this.authenticator.keyuri(
      userId.toString(),
      this.configService.getOrThrow<string>('app.name', { infer: true }) ?? '',
      secret,
    );
